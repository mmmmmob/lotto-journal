...
class CheckResultViewModel {
    ...
    func numberSearchAPI(searchNum: String, date: String) {
        Task {
            await numberSearch(searchNum: searchNum, date: date)
        }
    }
    
    func numberSearch(searchNum: String, date: String) async {
        do {
            ...
            print("Error in numberSearch: \(error)")
        }
    }
    ...
    func drawDateResultAPI(_ payload: Parameters) {
        Task {
            await drawDateResult(payload)
        }
    }
    
    func drawDateResult(_ payload: Parameters) async {
        do {
            let data = try await AF.request(
            ...
            // First Prize Neighbors clear and decode to struct
            ...
            print("Error in drawDateResult: \(error)")
        }
    }
    ...
    func latestResult() async {
        do {
            ...
            self.result.latestResultDate = json[pathLatestDate].string ?? "-"
            
            // Get Status
            let pathStatus: [JSONSubscriptType] = ["statusCode"]
            self.result.fetchLatestStatus = json[pathStatus].int ?? 500
        } catch {
            print("Error in latestResult: \(error)")
